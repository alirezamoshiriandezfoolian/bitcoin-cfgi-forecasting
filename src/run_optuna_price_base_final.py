...
from tuner_price import objective  # 📌 نسخه ویژه مدل قیمت


# 1️⃣ ساخت پوشه خروجی در صورت نبود
os.makedirs("optuna_results_price_base_final", exist_ok=True)


# 2️⃣ تعریف Study روی SQLite
study = optuna.create_study(
    study_name="bilstm_price_tuning_base_final",
    direction="minimize",
    storage="sqlite:///optuna_results_price_base_final/optuna_results_price_base_final.db",
    load_if_exists=True
)
...
best_params_path = "optuna_results_price_base_final/best_params_price_base_final.pkl"
...
try:
    ...
    plt.tight_layout()
    plt.savefig("optuna_results_price_base_final/optimization_history.png")
    plt.close()
...
try:
    ...
    plt.tight_layout()
    plt.savefig("optuna_results_price_base_final/param_importances.png")
    plt.close()
...
try:
    ...
    plt.tight_layout()
    plt.savefig("optuna_results_price_base_final/parallel_coordinates.png")
    plt.close()
